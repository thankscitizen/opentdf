...
To access, `tilt up` and navigate to `https://localhost:65432/abacus`.

![Tilt dashboard, with the abacus resource and its link displayed](static/tilt-abacus-link.png)
...
Log in with the credentials generated by the [values fed into the bootstrap script](helm/opentdf/charts/bootstrap/values.yaml#L10-L11), currently `keycloakadmin`. The corresponding password is found there as well.
