...
from opentdf import NanoTDFClient, OIDCCredentials, LogLevel, TDFStorageType
...
class WholeBoard(BaseModel):
    player1: conlist(
        conlist(str, min_items=10, max_items=10), min_items=10, max_items=10
    )
    player2: conlist(
        conlist(str, min_items=10, max_items=10), min_items=10, max_items=10
    )
        ...
        for i in range(len(self.board)):
            ...
            for j in range(len(self.board[i])):
                client = NanoTDFClient(
                    oidc_credentials=oidc_creds, kas_url=EXTERNAL_KAS_URL
                )
                ...
                string_storage = TDFStorageType()
                string_storage.set_tdf_storage_string_type(self.board[i][j])
                encrypt_string = client.encrypt_data(string_storage)
...
class Game:
    ...
    def getWholeBoard(self):
        ...
        return {
            "player1": self.player1.board_encrypted if self.player1 is not None else [],
...
def _checkBoard(board):
    return (
        _findCarrier(board)
        and _findBattleship(board)
        and _findCruiser(board)
        and _findDestroyer(board)
        and _findSubmarine(board)
    )
        ...
        for pos_num in range(SIZE):
            if board[row_num][pos_num] == title:
                ...
                if (pos_num + length - 1) <= (SIZE - 1):
                    # could be horizontal
                    if [title for _ in range(length)] == board[row_num][
                        pos_num : pos_num + length
                    ]:
                        valid_ship = True
                        # remove the ship
                        new_board = board.copy()
                        new_board[row_num][pos_num : pos_num + length] = [
                            OCEAN for _ in range(length)
                        ]
                # check if it could be vertical
                if (not valid_ship) and (row_num + length - 1) <= (SIZE - 1):
                    if [title for _ in range(length)] == [
                        board[x][pos_num] for x in range(row_num, row_num + length)
                    ]:
                        valid_ship = True
                        # remove the ship
                        new_board = board.copy()
...
def _findBattleship(board):
    return _findShips(
        board, BATTLESHIP.size, BATTLESHIP.name, SHIPS.count(BATTLESHIP), 0
    )
